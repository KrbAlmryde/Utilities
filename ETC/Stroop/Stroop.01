...
3dToutcount -automask 01_Tcat/${subj}_${run}_tcat+orig > 02_Tshift/precount_${subj}_${run}.1D

1dplot -one   02_Tshift/precount_${subj}_${run}.1D

3dDespike -prefix 02_Tshift/${subj}_${run}_despike -ssave 02_Tshift/${subj}_${run}_spike 01_Tcat/${subj}_${run}_tcat+orig

3dTshift -tzero 0 -rlt+ -quintic -prefix 02_Tshift/${subj}_${run}_tshift      \
             02_Tshift/${subj}_${run}_despike+orig

3dToutcount -automask 02_Tshift/${subj}_${run}_tshift+orig > 02_Tshift/postcount_${subj}_${run}.1D
...
cat 03_VolReg/dfile_${subj}_${run}_1D > 03_VolReg/${subj}_${run}_1D

mkdir $results_dir/04_Merge
#-------------------------------------------------------
echo "blur each volume"

    3dmerge -1blur_fwhm 8.0 -doall -prefix 04_Merge/${subj}_${run}_blur   \
...
3dDeconvolve -input 06_Norm/${subj}_${run}_norm+orig.HEAD                     \
   ...
   -stim_label 1 07_Deconvolve/${subj}_${run}_mvfw_res                                       \
   -stim_times 2 00_Data/${subj}_${run}_fvmw_res.txt 'GAM'                  \
   -stim_label 2 07_Deconvolve/${subj}_${run}_fvmw_res                                       \
   -stim_times 3 00_Data/${subj}_${run}_mvmw_res.txt 'GAM'                  \
   -stim_label 3 07_Deconvolve/${subj}_${run}_mvmw_res                                       \
   -stim_times 4 00_Data/${subj}_${run}_mvnw_res.txt 'GAM'                  \
   -stim_label 4 07_Deconvolve/${subj}_${run}_mvnw_res                                       \
   -stim_times 5 00_Data/${subj}_${run}_fvnw_res.txt 'GAM'                 \
   -stim_label 5 07_Deconvolve/${subj}_${run}_fvnw_res                                      \
   -stim_times 6 00_Data/${subj}_${run}_fvfw_res.txt 'GAM'                 \
   -stim_label 6 07_Deconvolve/${subj}_${run}_fvfw_res                                      \
   -stim_times 7 00_Data/${subj}_${run}_null_res.txt 'GAM'                  \
   -stim_label 7 07_Deconvolve/${subj}_${run}_null_res                                       \
...
#    -gltsym 'SYM: + ${subj}_${run}_fvfw_res - ${subj}_${run}_null_res'	\
   -fout -tout -x1D 07_Deconvolve/X.xmat.1D -xjpeg 07_Deconvolve/X.jpg                            \
   -fitts 07_Deconvolve/${subj}_${run}_fitts                                                \
   -bucket 07_Deconvolve/${subj}_${run}_stats


#create an all_runs dataset to match the fitts, errts, etc.

3dTcat -prefix 07_Deconvolve/${subj}_${run}_all_runs 06_Norm/${subj}_${run}_norm+orig.HEAD

#create ideal files for each stim run
1dcat 07_Deconvolve/X.xmat.1D'[6]' > 07_Deconvolve/ideal_mvfw-${run}-res.1D
1dcat 07_Deconvolve/X.xmat.1D'[7]' > 07_Deconvolve/ideal_fvmw-${run}-res.1D
1dcat 07_Deconvolve/X.xmat.1D'[8]' > 07_Deconvolve/ideal_mvmw-${run}-res.1D
1dcat 07_Deconvolve/X.xmat.1D'[9]' > 07_Deconvolve/ideal_mvnw-${run}-res.1D
1dcat 07_Deconvolve/X.xmat.1D'[10]' > 07_Deconvolve/ideal_fvnw-${run}-res.1D
1dcat 07_Deconvolve/X.xmat.1D'[11]' > 07_Deconvolve/ideal_fvfw-${run}-res.1D
1dcat 07_Deconvolve/X.xmat.1D'[12]' > 07_Deconvolve/ideal_null-${run}-res.1D
...
3dbucket -prefix 08_AnovaPrep/${subj}_${run}_mvfw_peak_irf -fbuc 07_Deconvolve/${subj}_${run}_mvfw_norm_irf+orig'[8]'
3dbucket -prefix 08_AnovaPrep/${subj}_${run}_fvmw_peak_irf -fbuc 07_Deconvolve/${subj}_${run}_fvmw_norm_irf+orig'[8]'
3dbucket -prefix 08_AnovaPrep/${subj}_${run}_mvmw_peak_irf -fbuc 07_Deconvolve/${subj}_${run}_mvmw_norm_irf+orig'[8]'
3dbucket -prefix 08_AnovaPrep/${subj}_${run}_mvnw_peak_irf -fbuc 07_Deconvolve/${subj}_${run}_mvnw_norm_irf+orig'[8]'
3dbucket -prefix 08_AnovaPrep/${subj}_${run}_fvnw_peak_irf -fbuc 07_Deconvolve/${subj}_${run}_fvnw_norm_irf+orig'[8]'
3dbucket -prefix 08_AnovaPrep/${subj}_${run}_fvfw_peak_irf -fbuc 07_Deconvolve/${subj}_${run}_fvfw_norm_irf+orig'[8]'

#-------------------------------------------------------
# Warp datasets to Talairach space
adwarp -apar 00_Data/${subj}_spgr+tlrc -dpar 08_AnovaPrep/${subj}_${run}_mvfw_peak_irf+orig
adwarp -apar 00_Data/${subj}_spgr+tlrc -dpar 08_AnovaPrep/${subj}_${run}_fvmw_peak_irf+orig
adwarp -apar 00_Data/${subj}_spgr+tlrc -dpar 08_AnovaPrep/${subj}_${run}_mvmw_peak_irf+orig
adwarp -apar 00_Data/${subj}_spgr+tlrc -dpar 08_AnovaPrep/${subj}_${run}_mvnw_peak_irf+orig
adwarp -apar 00_Data/${subj}_spgr+tlrc -dpar 08_AnovaPrep/${subj}_${run}_fvnw_peak_irf+orig
adwarp -apar 00_Data/${subj}_spgr+tlrc -dpar 08_AnovaPrep/${subj}_${run}_fvfw_peak_irf+orig
