...
##################################################################################################
	...
	3dcalc -verbose -float -a ${subrun}.blur.nii -b ${subrun}.mean.nii -c ${subrun}.automask.nii \
		-expr 'c * min(200, a/b*100)' -prefix ${subrun}.scale.nii.gz
...
##################################################################################################
	...
	1d_tool.py -infile ${subrun}.dfile.1D -set_nruns 1 -set_tr 3.5 -show_censor_count \
		-censor_prev_TR -censor_motion .1 motion.${subrun} -verb 2
